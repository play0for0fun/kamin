<!DOCTYPE html>
<html>
 <head>
  <meta content="text/html; charset=UTF-8" http-equiv=content-type>
  <meta name=viewport content="width=1260">
  <title>камин-москва.рф</title>
  <link rel="shortcut icon" href="img/favicon-1.png" type="image/x-icon">
  <style><? include('css/head.css'); ?></style>
  <script>
  !function(e,t,n){function r(){for(;u[0]&&"loaded"==u[0][l];)o=u.shift(),o[f]=!a.parentNode.insertBefore(o,a)}for(var i,s,o,u=[],a=e.scripts[0],f="onreadystatechange",l="readyState";i=n.shift();)s=e.createElement(t),"async"in a?(s.async=!1,e.head.appendChild(s)):a[l]?(u.push(s),s[f]=r):e.write("<"+t+' src="'+i+'"></'+t+">"),s.src=i}(document,"script",
  ["js/libs.min.js",
  "js/main.js",
  "https://maps.googleapis.com/maps/api/js?sensor=false",
  "js/map.js"]);
  </script>
  <noindex><script async src="data:text/javascript;charset=utf-8;base64,"></script></noindex>
 </head>
 <body>
  <div id="loader-wrapper">
   <div id="loader"></div>
   <div class="loader-section section-left"></div>
   <div class="loader-section section-right"></div>
  </div>
  <script>if(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)){document.getElementById("loader-wrapper").style.display="none"}</script>
  <div class="site-nav up"></div>
 <div class="site-nav down"></div>
  <div id="pages">
   <section class="header section">
    <header id="sec1">
     <div class="wrapper">
      <div class="logo"></div>
      <div class="phone">
       <div class="lptracker_phone">+7 (985) 227-04-49</div>
       <span><a href="#" id="zz-btn">заказать звонок</a></span>
      </div>
      <span class="header-text">Проектирование и монтаж каминов</span>
     </div>
    </header>
    <div class="wrapper">
     <div class="middle-wrap">
      <div class="slice" id="s1s1">
       <h2>За 5 дней смонтируем<br>безопасный камин под ключ <br><span>со стильной отделкой из натурального камня</span></h2>
      </div>
      <div class="slice" id="s1s2">
       <p class="garanty">100% гарантия безопасной изоляции</p>
      </div>
      <div class="gar-stamp"></div>
     </div>
    </div>
   </section>
  <section class="sec4 section" id="sec3">
    <div class="wrapper">
     <div class="middle-wrap">
      <div class="sec4-border-wrap">
       <div class="block-left">
        <h2>Главное - это<br>безопасность!</h2>
        <p>Мы находим выход<br>из любой ситуации!</p>
       </div>
       <div class="block-right">
        <div class="first-slider">
         <div class="first-slider-slide active" data-sld="0">
          <div class="sld-1-img" id="sld-1-img-1"></div>
          <div class="right">
           <h3>Установка камина в<br>деревяном доме</h3>
           <p class="sec4-text"><i>Поступил звонок клиента: "Что неужели установка камина в деревянном доме - прям так и не возможна?" Если не следить за тем, как горит огонь в камине, то - "Да" и задняя стенка не выдержала бы облицовку из камня, и мы нашли ИДЕАЛЬНОЕ решение, спереди камина расположили негорючее вещество, увеличили толщину жаропрочного силиката кальция для короба, а для задней стенки использовали специальные крепежи, которые предложил наш мастер, в итоге клиент остался очень доволен и порекомендовал нас ее друзьям!</i>
           </p>
          </div>
          <div class="block-left cont">
           <div class="number">1</div>
          </div>
         </div>
         <div class="first-slider-slide" data-sld="1">
          <div class="sld-1-img" id="sld-1-img-2"></div>
          <div class="right">
           <h3>Установка камина<br>под лестницей!</h3>
           <p class="sec4-text"><i>Клиент строил себе дом, и дизайнер проекта сильно напортачил тем, что не оставил места для дымохода камина, поэтому он решил сделать камин на улице, наша команда приехала на замер и только 2 часа клиент жаловался на дизайнера, мы его отговорили от ненужной идеи и нашли идеальное решение - установили камин под лестницей, сделав специальное отверстие в стене, сколько было радости у клиента!)</i>
           </p>
          </div>
          <div class="block-left cont">
           <div class="number">2</div>
          </div>
         </div>
         <div class="first-slider-slide" data-sld="2">
          <div class="sld-1-img" id="sld-1-img-3"></div>
          <div class="right">
           <h3>Проблема<br>поддымления</h3>
           <p class="sec4-text"><i>Смонтировали камин в доме<br>располооженом на крайнем к лесу<br>участке поселка. Однако камин при<br>открытом стекле топки поддымлял, что<br>недопустимо. Решили увеличитьт высоту<br>дымохода. Это не дало положительных<br>результатов. Поэтому мы внесли<br>изменение в конструктив топки (без<br>нарушения ее целостности). Что привело<br>к решению проблемы поддымления.</i>
           </p>
          </div>
          <div class="block-left cont">
           <div class="number">3</div>
          </div>
         </div>
        </div>
        <div class="arrow-left arr-l-abs"></div>
        <div class="arrow-right arr-r-abs"></div>
        <p class="prime">Самое важная часть работы
        <br> скрыта от ваших глаз</p>
       </div>
       <div class="s4-line"></div>
      </div>
     </div>
    </div>
   </section>
   <section class="sec2 section" id="sec2">
    <div class="wrapper">
     <div class="middle-wrap">
      <div class="slice" id="s2s1">
       <div class="block-cont" data-step="1">
        <h2>Узнайте стоимость вашего камина</h2>
        <div class="kalk-steps" data-step="1">
         <p>Выберите материал изготовления топки:</p>
         <div class="items">
          <div class="item" data-inp-name="st1">
           <img class="lazy" src="img/blank.png" data-original="./img/sec2_img1.jpg" alt="Сталь">
           <div class="item-text">
            <h3>СТАЛЬ</h3>
            <span>Дороже. Индивидуальные формы</span>
           </div>
          </div>
          <div class="item" data-inp-name="st1">
           <img class="lazy" src="img/blank.png" data-original="./img/sec2_img2.jpg" alt="Чугун">
           <div class="item-text">
            <h3>ЧУГУН</h3>
            <span>Дешевле. Стандартные формы</span>
           </div>
          </div>
         </div>
        </div>
        <div class="kalk-steps" data-step="2">
         <p>Выберите форму стекла:</p>
         <div class="items">
          <div class="item" data-inp-name="st2">
           <img class="lazy" src="img/blank.png" data-original="img/s2i1.jpg" alt="">
           <div class="item-text">
            <h4>Панарамное</h4>
           </div>
          </div>
          <div class="item" data-inp-name="st2">
           <img class="lazy" src="img/blank.png" data-original="img/s2i2.jpg" alt="">
           <div class="item-text">
            <h4>призма</h4>
           </div>
          </div>
          <div class="item" data-inp-name="st2">
           <img class="lazy" src="img/blank.png" data-original="img/s2i3.jpg" alt="">
           <div class="item-text">
            <h4>прямое</h4>
           </div>
          </div>
          <div class="item" data-inp-name="st2">
           <img class="lazy" src="img/blank.png" data-original="img/s2i4.jpg" alt="">
           <div class="item-text">
            <h4>сквозное</h4>
           </div>
          </div>
          <div class="item" data-inp-name="st2">
           <img class="lazy" src="img/blank.png" data-original="img/s2i5.jpg" alt="">
           <div class="item-text">
            <h4>угловое</h4>
           </div>
          </div>
          <div class="item" data-inp-name="st2">
           <img class="lazy" src="img/blank.png" data-original="img/s2i6.jpg" alt="">
           <div class="item-text">
            <h4>3 стекла</h4>
           </div>
          </div>
         </div>
        </div>
        <div class="kalk-steps" data-step="3">
         <p>Выберите материал изготовления:</p>
         <div class="items">
          <div class="item" data-inp-name="st3">
           <img class="lazy" src="img/blank.png" data-original="img/s3i1.jpg" alt="">
           <div class="item-text">
            <h4>травертин</h4>
            <p>от 50 $ / м2</p>
           </div>
          </div>
          <div class="item" data-inp-name="st3">
           <img class="lazy" src="img/blank.png" data-original="img/s3i3.jpg" alt="">
           <div class="item-text">
            <h4>мрамор</h4>
            <p>от 70 $ / м2</p>
           </div>
          </div>
           <div class="item" data-inp-name="st3">
           <img class="lazy" src="img/blank.png" data-original="img/s3i2.jpg" alt="">
           <div class="item-text">
            <h4>Гранит</h4>
            <p>от 100 $ / м2</p>
           </div>
          </div>
          <div class="item" data-inp-name="st3">
           <img class="lazy" src="img/blank.png" data-original="img/s3i4.jpg" alt="">
           <div class="item-text">
            <h4>оникс</h4>
            <p>от 150 $ / м2</p>
           </div>
          </div>
         </div>
        </div>
        <div class="kalk-steps" data-step="4">
         <p class="s2-form-p">Заполните форму и мы подберем для вас<br>оборудование и оценим работу:</p>
         <form action="ajax/mail.php" method="post">
          <input type="hidden" name="frm" value="Подбор камина">
          <input type="text" name="name" placeholder="Введите имя" />
          <input type="text" name="phone" placeholder="Ваш телефон" />
          <input type="hidden" name="st1">
          <input type="hidden" name="st2">
          <input type="hidden" name="st3">
          <button type="submit">ОТПРАВИТЬ</button>
         </form>
        </div>
        
        <span class="back">НАЗАД</span>
        <span class="nav"><span class="cursor"></span></span>
       </div>
      </div>
     </div>
    </div>
   </section>
   <section class="sec5 section" id="sec4">
    <div class="wrapper">
     <div class="middle-wrap">
      <div class="slice" id="s5s1">
       <ul class="block_left">
        <li class="shield">Гарантия на<br> оборудование</li>
        <li class="document">Сроки выполнения и<br> условия их соблюдения<br>прописываются в договоре</li>
       </ul>
       <div class="sec5_line"></div>
       <div class="block_right">
        <h2>Гарантия качества<br>и сроков работ</h2>
        <p>Мы используем только<br> высококачественные топки и печи:</p>
        <items>
        <item><img class="lazy" src="img/blank.png" data-original="./img/part-logo2.png" alt=""></item>
        <item><img class="lazy" src="img/blank.png" data-original="./img/part-logo5.png" alt=""></item>
        <item><img class="lazy" src="img/blank.png" data-original="./img/part-logo6.png" alt=""></item>
        <item><img class="lazy" src="img/blank.png" data-original="./img/part-logo1.png" alt=""></item>
        <item><img class="lazy" src="img/blank.png" data-original="./img/part-logo3.png" alt=""></item>
        <item><img class="lazy" src="img/blank.png" data-original="./img/part-logo4.png" alt=""></item>
        </items>
       </div>
      </div>
     </div>
    </div>
   </section>
   <section class="sec6 section" id="sec5">
    <div class="wrapper">
     <div class="middle-wrap">
      <div class="slice" id="s6s1">
       <div>
        <h2>шеф-монтаж и контроль качества <br> всего за <span>15 000 руб.</span></h2>
        <ul class="list">
         <li class="list-li">Дизайн проекта
          <br> камина
         </li>
         <li class="list-li">Подбор качественного
          <br> оборудования
         </li>
         <li class="list-li">Закупка оборудования по
          <br> ценам от производителя
         </li>
        </ul>
        <ul class="list">
         <li class="list-li" id="ttt">Устройство
          <br> пожаробезопасной
          <br> конструкции
         </li>
         <!--<li class="list-li">Создание дизайн-
          <br> макета облицовки
         </li>-->
         <li class="list-li">Контроль качества
          <br> работ до сдачи
         </li>
         <li class="list-li">Подбор и доставка
          <br> облицовочного камня
         </li>
        </ul>
        <ul class="list">
         
        </ul>
        <div class="clear"></div>
       </div>
       <div class="full-block">
        <img class="lazy" src="img/blank.png" data-original="./img/prorab.png" alt="">
        <p>Получите консультацию
         <br> от профессионала
        <br> с опытом 12 лет прямо сейчас !</p>
        <form action="ajax/mail.php" method="post">
         <input type="hidden" name="frm" value="Получите консультацию">
         <input type="text" name="name" placeholder="Введите имя" />
         <input type="text" name="phone" placeholder="Ваш телефон" />
         <button type="submit">ОТПРАВИТЬ</button>
        </form>
       </div>
      </div>
     </div>
    </div>
   </section>
   <section class="sec7 section" id="sec6">
    <div class="wrapper">
     <div class="middle-wrap">
      <div class="sec7-border-wrap">
       <div class="slice" id="s7s1">
        <h2>Собственный склад натурального камня</h2>
       </div>
       <div class="slice" id="s7s2">
        <div class="full-block">
         <ul class="left">
          <li class="stone-pager" data-page="1">Травертин</li>
          <li class="stone-pager" data-page="2">Гранит</li>
          <li  class="stone-pager active" data-page="3">Мрамор</li>
          <li class="stone-pager" data-page="4">Оникс</li>
         </ul>
         <div class="right">
          <div class="stone-wrap" data-page="1">
           <div class="item stone-item" data-tooltip="#b1i1"><img class="lazy" src="img/blank.png" data-original="./img/stone/st1_1.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b1i2"><img class="lazy" src="img/blank.png" data-original="./img/stone/st1_2.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b1i3"><img class="lazy" src="img/blank.png" data-original="./img/stone/st1_3.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b1i4"><img class="lazy" src="img/blank.png" data-original="./img/stone/st1_4.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b1i5"><img class="lazy" src="img/blank.png" data-original="./img/stone/st1_5.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b1i6"><img class="lazy" src="img/blank.png" data-original="./img/stone/st1_6.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b1i7"><img class="lazy" src="img/blank.png" data-original="./img/stone/st1_7.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b1i8"><img class="lazy" src="img/blank.png" data-original="./img/stone/st1_8.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b1i9"><img class="lazy" src="img/blank.png" data-original="./img/stone/st1_9.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b1i10"><img class="lazy" src="img/blank.png" data-original="./img/stone/st1_10.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b1i11"><img class="lazy" src="img/blank.png" data-original="./img/stone/st1_11.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b1i12"><img class="lazy" src="img/blank.png" data-original="./img/stone/st1_12.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b1i13"><img class="lazy" src="img/blank.png" data-original="./img/stone/st1_13.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b1i14"><img class="lazy" src="img/blank.png" data-original="./img/stone/st1_14.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b1i15"><img class="lazy" src="img/blank.png" data-original="./img/stone/st1_15.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b1i16"><img class="lazy" src="img/blank.png" data-original="./img/stone/st1_16.jpg" alt="камень"></div>
           <div class="clear"></div>
           <div class="item"><span>от 50 $ / м<sup>2</sup></span></div>
           <div class="item"><span>от 70 $ / м<sup>2</sup></span></div>
           <div class="item"><span>от 90 $ / м<sup>2</sup></span></div>
           <div class="item"><span>от 100 $ / м<sup>2</sup></span></div>
          </div>
          <div class="stone-wrap" data-page="2">
           <div class="item stone-item" data-tooltip="#b2i1"><img class="lazy" src="img/blank.png" data-original="./img/stone/st2_1.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b2i2"><img class="lazy" src="img/blank.png" data-original="./img/stone/st2_2.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b2i3"><img class="lazy" src="img/blank.png" data-original="./img/stone/st2_3.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b2i4"><img class="lazy" src="img/blank.png" data-original="./img/stone/st2_4.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b2i5"><img class="lazy" src="img/blank.png" data-original="./img/stone/st2_5.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b2i6"><img class="lazy" src="img/blank.png" data-original="./img/stone/st2_6.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b2i7"><img class="lazy" src="img/blank.png" data-original="./img/stone/st2_7.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b2i8"><img class="lazy" src="img/blank.png" data-original="./img/stone/st2_8.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b2i9"><img class="lazy" src="img/blank.png" data-original="./img/stone/st2_9.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b2i10"><img class="lazy" src="img/blank.png" data-original="./img/stone/st2_10.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b2i11"><img class="lazy" src="img/blank.png" data-original="./img/stone/st2_11.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b2i12"><img class="lazy" src="img/blank.png" data-original="./img/stone/st2_12.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b2i13"><img class="lazy" src="img/blank.png" data-original="./img/stone/st2_13.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b2i14"><img class="lazy" src="img/blank.png" data-original="./img/stone/st2_14.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b2i15"><img class="lazy" src="img/blank.png" data-original="./img/stone/st2_15.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b2i16"><img class="lazy" src="img/blank.png" data-original="./img/stone/st2_16.jpg" alt="камень"></div>
           <div class="clear"></div>
           <div class="item"><span>от 100 $ / м<sup>2</sup></span></div>
           <div class="item"><span>от 250 $ / м<sup>2</sup></span></div>
           <div class="item"><span>от 450 $ / м<sup>2</sup></span></div>
           <div class="item"><span>от 550 $ / м<sup>2</sup></span></div>
          </div>
          <div class="stone-wrap" data-page="3">
           <div class="item stone-item" data-tooltip="#b3i1"><img class="lazy" src="img/blank.png" data-original="./img/stone/st3_1.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b3i2"><img class="lazy" src="img/blank.png" data-original="./img/stone/st3_2.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b3i3"><img class="lazy" src="img/blank.png" data-original="./img/stone/st3_3.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b3i4"><img class="lazy" src="img/blank.png" data-original="./img/stone/st3_4.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b3i5"><img class="lazy" src="img/blank.png" data-original="./img/stone/st3_5.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b3i6"><img class="lazy" src="img/blank.png" data-original="./img/stone/st3_6.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b3i7"><img class="lazy" src="img/blank.png" data-original="./img/stone/st3_7.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b3i8"><img class="lazy" src="img/blank.png" data-original="./img/stone/st3_8.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b3i9"><img class="lazy" src="img/blank.png" data-original="./img/stone/st3_9.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b3i10"><img class="lazy" src="img/blank.png" data-original="./img/stone/st3_10.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b3i11"><img class="lazy" src="img/blank.png" data-original="./img/stone/st3_11.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b3i12"><img class="lazy" src="img/blank.png" data-original="./img/stone/st3_12.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b3i13"><img class="lazy" src="img/blank.png" data-original="./img/stone/st3_13.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b3i14"><img class="lazy" src="img/blank.png" data-original="./img/stone/st3_14.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b3i15"><img class="lazy" src="img/blank.png" data-original="./img/stone/st3_15.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b3i16"><img class="lazy" src="img/blank.png" data-original="./img/stone/st3_16.jpg" alt="камень"></div>
           <div class="clear"></div>
           <div class="item"><span>от 70 $ / м<sup>2</sup></span></div>
           <div class="item"><span>от 100 $ / м<sup>2</sup></span></div>
           <div class="item"><span>от 200 $ / м<sup>2</sup></span></div>
           <div class="item"><span>от 500 $ / м<sup>2</sup></span></div>
          </div>
          <div class="stone-wrap" data-page="4">
           <div class="item stone-item" data-tooltip="#b4i1"><img class="lazy" src="img/blank.png" data-original="./img/stone/st4_1.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b4i2"><img class="lazy" src="img/blank.png" data-original="./img/stone/st4_2.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b4i3"><img class="lazy" src="img/blank.png" data-original="./img/stone/st4_3.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b4i4"><img class="lazy" src="img/blank.png" data-original="./img/stone/st4_4.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b4i5"><img class="lazy" src="img/blank.png" data-original="./img/stone/st4_5.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b4i6"><img class="lazy" src="img/blank.png" data-original="./img/stone/st4_6.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b4i7"><img class="lazy" src="img/blank.png" data-original="./img/stone/st4_7.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b4i8"><img class="lazy" src="img/blank.png" data-original="./img/stone/st4_8.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b4i9"><img class="lazy" src="img/blank.png" data-original="./img/stone/st4_9.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b4i10"><img class="lazy" src="img/blank.png" data-original="./img/stone/st4_10.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b4i11"><img class="lazy" src="img/blank.png" data-original="./img/stone/st4_11.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b4i12"><img class="lazy" src="img/blank.png" data-original="./img/stone/st4_12.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b4i13"><img class="lazy" src="img/blank.png" data-original="./img/stone/st4_13.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b4i14"><img class="lazy" src="img/blank.png" data-original="./img/stone/st4_14.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b4i15"><img class="lazy" src="img/blank.png" data-original="./img/stone/st4_15.jpg" alt="камень"></div>
           <div class="item stone-item" data-tooltip="#b4i16"><img class="lazy" src="img/blank.png" data-original="./img/stone/st4_16.jpg" alt="камень"></div>
           <div class="clear"></div>
           <div class="item"><span>от 150 $ / м<sup>2</sup></span></div>
           <div class="item"><span>от 200 $ / м<sup>2</sup></span></div>
           <div class="item"><span>от 400 $ / м<sup>2</sup></span></div>
           <div class="item"><span>от 550 $ / м<sup>2</sup></span></div>
          </div>
         </div>
        </div>
       </div>
      </div>
     </div>
    </div>
   </section>
   <section class="sec8 section" id="sec7">
    <div class="wrapper">
     <div class="middle-wrap">
      <div class="slice" id="s8s1">
       <h2>За 12 лет работы</h2>
       <p>мы установили сотни каминов!</p>
       <div class="s8-line"></div>
      </div>
     </div>
    </div>
   </section>
   <section class="sec9 section" id="sec8">
    <div class="wrapper">
     <div class="middle-wrap">
      <div class="s9-border-wrap">
       <div class="left">
        <h2>О нашей компании<br> в цифрах</h2>
        <div class="s9-line-b-3"></div>
       </div>
       <div class="right">
        <div class="s9-el s9-el-1">
         <span class="big">317</span>
         <p class="ch2">каминов<br>установлено</p>
         <div class="s9-line-b-1"></div>
        </div>
        <div class="s9-el s9-el-2">
         <p class="ch3">более<br>оттенков камней</p>
         <span class="ital">150</span>
        </div>
        <div class="s9-el s9-el-4">
         <span class="strong">47</span>
         <p class="ch4">обученных<br>монтажников</p>
        </div>
        <div class="s9-el s9-el-5">
         <p class="ch5">на рынке</p>
         <span class="small">12 <span>лет</span></span>
         <div class="s9-line-b-2"></div>
        </div>
        <div class="s9-comand"></div>
       </div>
      </div>
     </div>
    </div>
   </section>
   <section class="sec10 section" id="sec9">
    <div class="wrapper">
     <div class="middle-wrap">
      <div class="slice" id="s10s1">
       <h2>Уже есть готовые чертежи?</h2>
      </div>
      <div class="slice" id="s10s2">
       <p class="subtitle">Прикрепите их, заполните форму и мы сделаем<br> расчет стоимости за 1 час!</p>
      </div>
      <div class="slice" id="s10s3">
       <div class="s10-form">
        <div class="full-block" id="dropZone">
         <span>Нажмите </span><label class="btn-fu" for="file-input">ВЫБРАТЬ ДОКУМЕНТ</label>
         <input type="file" name="file-input" id="file-input"><br>
         <span>или перетащите в эту область ваш чертеж</span>
        </div>
        <div class="form-block">
         <h3>заполните форму и узнайте стоимость работ:</h3>
         <form action="ajax/mail.php" method="post" enctype="multipart/form-data">
          <input type="hidden" name="frm" value="Готовые чертежи">
          <input type="text" name="name" placeholder="Введите имя" />
          <input type="text" name="phone" placeholder="Ваш телефон" />
          <input type="hidden" name="file" id="filename" value="fileform"/>
          <button type="submit">ОТПРАВИТЬ</button>
         </form>
        </div>
       </div>
      </div>
     </div>
    </div>
   </section>
   <section class="sec11 section" id="sec10">
    <div class="wrapper">
     <div class="middle-wrap">
      <div class="slice" id="s11s1">
       <h2><span class="b-span">VIP вариант</span><br><span class="span">Мы создаем произведения искусства</span></h2>
      </div>
     </div>
    </div>
   </section>
   <!--section class="sec12 section">
   <div class="wrapper">
    <div class="middle-wrap">
     <div class="slice" id="s12s1">
      <div class="left">
       <h2 class="animation">отзвывы<br>счастливых<br>заказчиков</h2>
      </div>
      <div class="right">
       <div class="s12-d-block animation">
        <div class="second-slider">
         <div class="second-slider-slide active" data-sld="0">
          <div class="s12-slide"></div>
          <p>Игорь Владимирович, 24 года.<br> Камин премиум класса</p>
          <img class="lazy" src="img/blank.png" data-original="./img/letter.jpg" alt="Письмо">
         </div>
         <div class="second-slider-slide" data-sld="1">
          <div class="s12-slide"></div>
          <p>Игорь Владимирович, 24 года.<br> Камин премиум класса</p>
          <img class="lazy" src="img/blank.png" data-original="./img/letter.jpg" alt="Письмо">
         </div>
         <div class="second-slider-slide" data-sld="2">
          <div class="s12-slide"></div>
          <p>Игорь Владимирович, 24 года.<br> Камин премиум класса</p>
          <img class="lazy" src="img/blank.png" data-original="./img/letter.jpg" alt="Письмо">
         </div>
         <div class="second-slider-slide" data-sld="3">
          <div class="s12-slide"></div>
          <p>Игорь Владимирович, 24 года.<br> Камин премиум класса</p>
          <img class="lazy" src="img/blank.png" data-original="./img/letter.jpg" alt="Письмо">
         </div>
        </div>
       </div>
       <div class="arrows-second animation">
        <div class="arrow-right arr-r-second"></div>
        <div class="arrow-left arr-l-second"></div>
       </div>
       <div class="s12-line animation"></div>
      </div>
     </div>
    </div>
   </div>
  </section-->
  <section class="sec13 section" id="sec11">
   <div id="map"></div>
   <!--div class="wrapper">
   <div class="window">
    <ul>
     <li>Телефон:
      <br><span><a href="tel:88003004554">8 800 300 45 54</a></span></li>
      <li>Почта:
       <br><span>sales@kaminstroy.ru</span></li>
       <li>Адрес:
        <br><span>г. Москва, ул. Пресненская, <br>д.7, офис 12</span></li>
       </ul>
      </div-->
      <div class="write">
       <h2>остались вопросы?<br>пишите</h2>
       <form action="ajax/mail.php" method="post">
        <input type="hidden" name="frm" value="Остались вопросы?">
        <input type="text" name="name" placeholder="Введите имя" />
        <input type="text" name="phone" placeholder="Ваш телефон" />
        <textarea name="text" placeholder="Введите ваш вопрос"></textarea>
        <button type="submit">ОТПРАВИТЬ</button>
       </form>
      </div>
      <!--/div-->
      <footer>
       <div class="wrapper">
  <div class="logo_f"></div>
   <div class="foot_conf">   
    <a href="" class="confbtn">Политика конфиденциальности</a><br>
    Все права защищены
   </div>
   <a href="http://gulfstream.bz/" class="logo">Разработка сайта:</a>
  </div>
       </footer>
      </section>
     </div>
     <div id="hidden-box">
      <div class="pop" id="pop-zz">
       <h2>Заказать звонок</h2>
       <form action="ajax/mail.php" method="post">
        <input type="hidden" name="frm" value="Заказать звонок">
        <input type="text" name="name" placeholder="Введите имя" />
        <input type="text" name="phone" placeholder="Ваш телефон" />
        <button type="submit">ОТПРАВИТЬ</button>
       </form>
      </div>
      <div class="pop" id="okgo">
       <h2>Спасибо за заявку!<br>Наш менежер свяжется с Вами<br>в ближайшее время</h2>
      </div>
      <div class="popap" id="conf_pop">
    <h2>Политика конфиденциальности</h2>
<p>«Наша организация уважительно относится к правам клиента. Соблюдается строгая конфиденциальность при оформлении заказа. Сведения надёжно сохраняются и защищены от передачи.</p>

<p>Согласием на обработку данных клиента исключительно с целью оказания услуг является размещение заказа на сайте.</p>

<p>К персональным данным относится личная информация о клиенте: домашний адрес; имя, фамилия, отчество; сведения о рождении; имущественное, семейное положение; личные контакты (телефон, электронная почта) и прочие сведения, которые перечислены в Законе РФ № 152-ФЗ «О персональных данных» от 27 июля 2006 г.</p>

<p>Клиент вправе отказаться от обработки персональных данных. Нами в данном случае гарантируется удаление с сайта всех персональных данных в трёхдневный срок в рабочее время. Подобный отказ клиент может оформить простым электронным письмом на адрес, указанный на странице нашего сайта.</p>

   </div>
     </div>
     <div class="tooltips">
      <div class="how tooltip" id="b1i1">
       <h3>Ноче Вейн Кат</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Армения</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>50 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b1i2">
       <h3>Веди</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Армения</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>80 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b1i3">
       <h3>Аркобалено</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Испания</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>90 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b1i4">
       <h3>Белла Корелус</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>120 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b1i5">
       <h3>Классик Крос Кат</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Армения</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>50 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b1i6">
       <h3>Классик Лайт</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Турция</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>120 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b1i7">
       <h3>Сильвер</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Турция</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>110 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b1i8">
       <h3>Волнут</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Турция</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>110 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b1i9">
       <h3>Ноче Крос Кат</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Армения</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>50 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b1i10">
       <h3>Классик Медиум</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Турция</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>80 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b1i11">
       <h3>Мистик</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Армения</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>90 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b1i12">
       <h3>Голд</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>150 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b1i13">
       <h3>Антик</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Армения</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>50 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b1i14">
       <h3>Еллоу</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Турция</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>70 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b1i15">
       <h3>Голд</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Турция</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>105 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b1i16">
       <h3>Ред</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Испания</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>110 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b2i1">
       <h3>Тропикал Фэшион</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Бразилия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>225 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b2i2">
       <h3>Центуриус</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Бразилия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>250 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b2i3">
       <h3>Фьюжн</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Бразилия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>450 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b2i4">
       <h3>Сандал</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Бразилия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>580 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b2i5">
       <h3>Титаниум</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Испания</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>150 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b2i6">
       <h3>Хариикейн</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Бразилия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>220 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b2i7">
       <h3>Лейк Плэсид</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Бразилия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>450 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b2i8">
       <h3>Мулен Руж</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Бразилия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>550 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b2i9">
       <h3>Тайгер Скин Еллоу</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Китай</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>100 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b2i10">
       <h3>Тайкунблю</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Бразилия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>320 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b2i11">
       <h3>Азул Макаубус</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Бразилия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>500 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b2i12">
       <h3>Айс Перл</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Бразилия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>720 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b2i13">
       <h3>Таун Браун</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Индия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>110 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b2i14">
       <h3>Сплэндер Голд</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Бразилия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>300 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b2i15">
       <h3>Россо Фиорентино</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Бразилия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>500 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b2i16">
       <h3>Азул Бахия</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Бразилия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>550 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b3i1">
       <h3>Крема Марфил</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>85 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b3i2">
       <h3>Ботичино</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Испания</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>100 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b3i3">
       <h3>Россо Дамаско</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>300 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b3i4">
       <h3>Неро Порторо</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>700 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b3i5">
       <h3>Форест Голд</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Индия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>85 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b3i6">
       <h3>Имперадор Дарк</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>90 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b3i7">
       <h3>Россо Франция</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Франция</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>300 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b3i8">
       <h3>Калаката Виоло</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>450 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b3i9">
       <h3>Верде Гуатемала</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Испания</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>80 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b3i10">
       <h3>Имперадор Лайт</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Турция</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>90 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b3i11">
       <h3>Арабекато</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>300 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b3i12">
       <h3>Айс Кристал</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Бразилия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>500 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b3i13">
       <h3>Форест Грин</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Индия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>85 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b3i14">
       <h3>Неро Марквина</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Испания</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>170 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b3i15">
       <h3>Блэк Фоссил</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>200 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b3i16">
       <h3>Джиало Сиенна</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>700 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b4i1">
       <h3>Авион</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>150 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b4i2">
       <h3>Алабастро</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>200 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b4i3">
       <h3>Арко Ирис</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>500 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b4i4">
       <h3>Алабастро Дарк</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>550 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b4i5">
       <h3>Бока Де Рио</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>150 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b4i6">
       <h3>Верде Пакистано</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>200 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b4i7">
       <h3>Виола</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>400 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b4i8">
       <h3>Амбер</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>500 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b4i9">
       <h3>Вайт</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>150 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b4i10">
       <h3>Дарк Пакистано</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Турция</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>200 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b4i11">
       <h3>Капучино</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>400 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b4i12">
       <h3>Ардженто</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>600 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b4i13">
       <h3>Вайт Еллоу</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Армения</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>150 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b4i14">
       <h3>Эмпайрл Голд</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>250 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b4i15">
       <h3>Матисс</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>450 $/м<sup>2</sup> </b></p>
      </div>
      <div class="how tooltip" id="b4i16">
       <h3>Вулкано</h3>
       <p class="how_p">Страна производитель:<br>
       <b>Италия</b></p>
       <p class="how_p">Стоимость за кв.м:<br>
       <b>550 $/м<sup>2</sup> </b></p>
      </div>
     </div>
    <div class="menu-btn"></div>
    <div class="menu">
        <a href="#sec1" class="menu-a">Главная</a>
        <a href="#sec3" class="menu-a">Безопасность</a>
        <a href="#sec2" class="menu-a">Стоимость</a>        
        <a href="#sec4" class="menu-a">Гарантии</a>
        <a href="#sec5" class="menu-a">Шеф-монтаж</a>
        <a href="#sec6" class="menu-a">Облицовка</a>
        <a href="#sec8" class="menu-a">О компании</a>
        <a href="#sec10" class="menu-a">VIP вариант</a>
        <a href="#sec11" class="menu-a">Контакты</a>
    </div>
     <!-- Yandex.Metrika counter --> <script type="text/javascript"> (function (d, w, c) { (w[c] = w[c] || []).push(function() { try { w.yaCounter36722730 = new Ya.Metrika({ id:36722730, clickmap:true, trackLinks:true, accurateTrackBounce:true, webvisor:true, trackHash:true }); } catch(e) { } }); var n = d.getElementsByTagName("script")[0], s = d.createElement("script"), f = function () { n.parentNode.insertBefore(s, n); }; s.type = "text/javascript"; s.async = true; s.src = "https://mc.yandex.ru/metrika/watch.js"; if (w.opera == "[object Opera]") { d.addEventListener("DOMContentLoaded", f, false); } else { f(); } })(document, window, "yandex_metrika_callbacks"); </script> <noscript><div><img src="https://mc.yandex.ru/watch/36722730" style="position:absolute; left:-9999px;" alt="" /></div></noscript> <!-- /Yandex.Metrika counter -->
    </body>
   </html>