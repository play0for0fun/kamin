...
if (isMobile == true) {
  ...
  $('<style>..animation,.animation2,.animation3,.animation4 *{opacity:1;transition:0s all 0s!important;animation-duration:0s!important;animation-delay:0s!important}.site-nav.down, .site-nav.up{display:none!important}section{height:800px!important}</style>').appendTo('head');
}
   ...
   $('#pages').fullpage({
      ...
      afterLoad: function(anchorLink, index){
           ...
           // if(index == 3 || index == 4 || index == 6){$('.site-nav.down').addClass('black');}else{$('.site-nav.down').removeClass('black');}
            if(index == 11){$('.site-nav.up').addClass('black');}else{$('.site-nav.up').removeClass('black');} 
...
slider2 = $('.second-slider').bxSlider({pager:false,controls:false, auto:false, speed: 400,
    onSlideNext:function($slideElement, oldIndex, newIndex){
      ...
      onSliderLoad:function(){
        ...
        console.log('second-slider-init');
      }});
